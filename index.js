const loadCategory = async () => {
  ...
  navbar(data.results[5].category);
};
...
const loadTopNews = async () => {
  const res = await fetch(
    `https://newsdata.io/api/1/latest?country=bd&category=top&apikey=pub_55664120963394010ff7d91bc73d25d8e3cde`
  );
  const data = await res.json();
  topContent(data.results);
};
loadTopNews();

// load top content

const topNewsContainer = document.getElementById("news-item");
function topContent(items) {
  const modalContainer = document.getElementById("modal");
  console.log(items);
  ...
  items.forEach((item) => {
    ...
    newsItem.innerHTML = `
      ...
      `;
    topNewsContainer.prepend(newsItem);
